...
print("=" * 80)
print("M4 ADVERSARIAL FINE-TUNING (FULL TRAINING)")
print("=" * 80)
...
weight_decay = float(m4_cfg.get("weight_decay", 0.01))
...
print(f"Config: model={model_name}, batch={batch_size}, lr={learning_rate}, epochs={epochs}")
...
ckpt_path = PROJECT_ROOT / "outputs" / "m3" / "smoke" / "best_model.pt"
if not ckpt_path.exists():
    ckpt_path = PROJECT_ROOT / "outputs" / "m3" / "best_model.pt"
checkpoint = torch.load(str(ckpt_path), map_location="cpu")
model.load_state_dict(checkpoint)
model.requires_grad_(True)
print(f"M3 checkpoint loaded from {ckpt_path}.")
...
result = trainer.train(
    ...
    save_dir=str(PROJECT_ROOT / "outputs" / "m4"),
...
best_state = torch.load(str(PROJECT_ROOT / "outputs" / "m4" / "best_model.pt"), map_location=device)
...
m3_checkpoint = torch.load(str(ckpt_path), map_location=device)
...
with open(PROJECT_ROOT / "outputs" / "m4" / "robustness_comparison.json", "w") as f:
    json.dump(robustness_comparison, f, indent=2, default=str)

with open(PROJECT_ROOT / "outputs" / "m4" / "clean_validation_metrics.json", "w") as f:
    json.dump(val_result["metrics"], f, indent=2, default=str)

with open(PROJECT_ROOT / "outputs" / "m4" / "adversarial_validation_metrics.json", "w") as f:
    json.dump(adv_val_result["metrics"], f, indent=2, default=str)

print("\nM4 training complete!")
print(f"Best checkpoint: {PROJECT_ROOT / 'outputs' / 'm4' / 'best_model.pt'}")